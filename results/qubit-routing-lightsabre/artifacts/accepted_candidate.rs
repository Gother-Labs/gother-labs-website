...
impl FrontLayerScores {
    ...
    fn max_distance(&self, topology: TopologyView<'_>) -> f64 {
        self.nodes
            .iter()
            .map(|pair| topology.distance(pair[0], pair[1]) as f64)
            .fold(0.0, f64::max)
...
impl Policy for CandidatePolicy {
    ...
    fn choose_best_swap(
        ...
        let lookahead_weight = scale(
            self.lookahead_weight * self.lookahead_scaling,
            extended_set.len(),
            self.set_scaling,
        );

        // Pre-filter swaps that have zero delta on the front layer.
        // These swaps will not improve the front layer score and should
        // be evaluated purely on the extended set (lookahead).
        let candidates: Vec<(usize, usize)> = candidates
            .into_iter()
            .filter(|swap| {
                let d = front_layer.score_delta(*swap, ctx.topology());
                // Keep swaps that have ANY effect on the front layer
                d.abs() > 1e-9
            })
            .collect();
        ...
        // Absolute score is constant regardless of the swap chosen,
        // so it can be computed once.
        ...
        // Apply decay to the absolute score (constant across candidates) to prioritize
        // early iterations or reset after decay_reset iterations.
        let decay = self.apply_decay();
        absolute_score *= decay;

        // Explicit tie-breaking: prioritize swaps that reduce the maximum distance
        // in the front layer. This is a stronger heuristic for CNOT reduction
        // than total distance alone and avoids the previous epsilon-based race conditions.
        let tie_break_weight = self.tie_break_weight;
        for (swap, score) in &mut swap_scores {
            let delta = front_layer.score_delta(*swap, ctx.topology());
            if delta < 0.0 {
                *score += delta * tie_break_weight;
            }
        }

        // Simple min-score selection. If min_score is still infinite (no candidates),
        // return None to avoid panic from indexing an empty vector.
        ...
        for (swap, score) in &swap_scores {
            ...
            if (*score - min_score).abs() < 1e-12 {
                best_swaps.push(*swap);
            }
        }

        // Safety check: if no valid swaps remain (e.g., all filtered out),
        // return None to avoid panic from indexing an empty vector.
